export async function propertyRoutes(request,db,session,{url,json,rows,text,HttpError,body}){
 ...
 if(request.method==='GET'&&path.startsWith('/api/buildings/')){
  ...
  const listings=await rows(db,'SELECT l.id,l.unit,l.rental_channel,l.landlord_id,p.name AS landlord_name,COUNT(r.id) AS reviews,AVG((r.maintenance+r.communication+r.deposit)/3.0) AS score FROM listings l JOIN landlords p ON p.id=l.landlord_id LEFT JOIN reviews r ON r.listing_id=l.id AND r.status=\'published\' WHERE l.building_id=?'+(unit?' AND l.unit=?':'')+' GROUP BY l.id ORDER BY l.unit',...unit?[id,unit]:[id]);
 ...
 if(request.method==='POST'&&path==='/api/listings'){
  ...
  const rentalChannel=input.rentalChannel;if(!['direct','agent'].includes(rentalChannel))throw new HttpError(400,'请选择房东直租或通过中介。');if(input.consent!==true)throw new HttpError(400,'请确认这条对应关系来自你的租住经历。');
  ...
  // An unnamed landlord record belongs to this Unit only; never merge by channel.
  const landlord={id:crypto.randomUUID()},landlordName='Unit '+unit+' · '+(rentalChannel==='direct'?'房东直租':'通过中介');statements.push(db.prepare('INSERT INTO landlords (id,name,city,district,created_at) VALUES (?,?,?,?,?)').bind(landlord.id,landlordName,building.city,building.address,Date.now()));
  const id=crypto.randomUUID();statements.push(db.prepare('INSERT INTO listings (id,building_id,unit,landlord_id,rental_channel,created_at) VALUES (?,?,?,?,?,?)').bind(id,building.id,unit,landlord.id,rentalChannel,Date.now()),db.prepare('INSERT INTO submissions (id,session,created_at) VALUES (?,?,?)').bind(crypto.randomUUID(),session.hash,Date.now()));
